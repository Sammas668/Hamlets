...
class BankSlotButton:
	...
	func _gui_input(event: InputEvent) -> void:
		# Only track drags when this slot actually has an item.
		# Do NOT require icon != null, because generated construction/building items
		# may be valid before their PNG exists.
		var has_item: bool = (String(get_meta("item_id", "")) != "")
	...
	# IMPORTANT:
	# We are using force_drag() manually, so we do not need Godot's automatic
	# _get_drag_data() pipeline here.
	...
	func _build_drag_data() -> Variant:
		...
		var tex: Texture2D = icon as Texture2D

		# Include both keys:
		# - "kind" keeps older drag receivers working.
		# - "drag_kind" is what the new building slot drop target expects.
		return {
			"kind": "bank_item",
			"drag_kind": "bank_item",
			"item_id": item_id,
	...
	func _build_drag_preview(data: Dictionary) -> Control:
		...
		# Fallback preview for items without icons, especially generated building items.
		var item_id := String(data.get("item_id", "Item"))
...
func _refresh() -> void:
	...
	for i in range(_cells.size()):
		...
		if i < items.size():
			...
			# Store the id on the button so drag data can read it.
...
func _init_cell_styleboxes() -> void:
	...
	# Pressed = normal. This removes the flash “ghost box”.
