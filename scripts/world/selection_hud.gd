...
func _apply_fragment(f: Node) -> void:
	...
	var mods_text_list: Array[String] = []

	if f is Fragment:
		var frag: Fragment = f as Fragment
		mods_text_list = _modifiers_to_strings(frag.modifiers)
		print("[SelectionHUD] frag.modifiers =", frag.modifiers)
	else:
		var mods_var: Variant = f.get("modifiers")
		mods_text_list = _variant_modifiers_to_strings(mods_var)

	# If still empty, try ResourceNodes
	if mods_text_list.is_empty() and typeof(ResourceNodes) != TYPE_NIL:
		if ResourceNodes.has_method("get_modifiers_for_tile"):
			...
			mods_text_list = _variant_modifiers_to_strings(rn_mods)

	var mods_text: String = "None"
	if mods_text_list.size() > 0:
		mods_text = ", ".join(mods_text_list)

	_set_text_control(_modifiers, "Modifiers: " + mods_text)
	print("[SelectionHUD] coord=", _current_coord, " mods(final)=", mods_text_list)

	_update_building_tab()


# ----------------- helpers (TOP-LEVEL, NOT inside _apply_fragment) -----------------

func _variant_modifiers_to_strings(v: Variant) -> Array[String]:
	var out: Array[String] = []
	if v is Array:
		for it in (v as Array):
			if it is Dictionary:
				out.append(_modifier_dict_to_string(it as Dictionary))
			else:
				out.append(String(it))
	return out


func _modifiers_to_strings(mods: Array) -> Array[String]:
	# Expected: Array[Dictionary] following your new system
	var out: Array[String] = []
	for it in mods:
		if it is Dictionary:
			out.append(_modifier_dict_to_string(it as Dictionary))
...
func _modifier_dict_to_string(d: Dictionary) -> String:
	# New naming system: { "name": String, "kind": String, "rarity": String, "skill": String? }
	var name := String(d.get("name", ""))
	if name == "":
		return "(unnamed)"

	# Keep it short: show name only (recommended)
	return name

	# If you ever want more detail, swap to:
